...
module.exports = {
	...
	async execute(interaction) {
		if (interaction.member.permissions.has(PermissionsBitField.Flags.ManageRoles)) {
			...
			const approve = new ButtonBuilder().setCustomId('button_verify_approve').setLabel('Approved').setStyle(ButtonStyle.Success).setDisabled(true) // Disable only this button
			...
			// Update the message (changes the button state)
			await interaction.update({
				components: [tools],
			})

			// Send an ephemeral message instead of reply()
			return interaction.followUp({
				content: 'fak you <:fak_you:1334747436748640358>',
				flags: MessageFlags.Ephemeral, // Keeps it private
			})
		} else {
			...
			// Since interaction.update() wasn't called, we can still use reply()
