...
const appeal = require('../modals/appeal')
...
if (!fs.existsSync(appealsFile)) {
	fs.writeFileSync(appealsFile, JSON.stringify([]))
}
...
const saveAppeals = appeals => {
	fs.writeFileSync(appealsFile, JSON.stringify(appeals, null, 2))
}
	...
	async execute(interaction) {
		...
		const modalHandlers = {
			model_verify: async () => {
				...
				logger.debug(`Verification submitted for user ${userId}`)
			},
			model_appeal: (model_appeal = async () => {
				const userId = interaction.user.id
				...
				let type = 'Unknown'
				const punishmentReason = interaction.fields.getTextInputValue('punishment_reason')
				const appealReason = interaction.fields.getTextInputValue('appeal_reason')

				const errorMessages = []

				try {
					const ban = await client.guilds.cache
						.get(config.guildId)
						.bans.fetch(userId)
						.catch(() => null)
					if (ban) {
					...
					logger.error(`Error checking punishment status for ${userId}:`, error)
				}

				if (errorMessages.length > 0) {
					const errorEmbed = new EmbedBuilder().setTitle('Error | Submission Invalid').setDescription(errorMessages.join('\n')).setColor(config.embeds.colors.error).setTimestamp()

					appeals = appeals.filter(id => id !== userId)
					saveAppeals(appeals)

					logger.error(`Appeal failed for user ${userId}: ${errorMessages.join(', ')}`)
				...
				const logEmbed = new EmbedBuilder()
					...
					.addFields({ name: 'Punishment Type', value: type }, { name: 'Punishment Reason', value: punishmentReason }, { name: 'Appeal Reason', value: appealReason })
					.setColor(config.embeds.colors.default)
					.setTimestamp()
					.setFooter({ text: `User ID: ${interaction.user.id}` })

				const approve = new ButtonBuilder().setCustomId('button_appeal_approve').setLabel('Approve').setStyle(ButtonStyle.Success)

				const deny = new ButtonBuilder().setCustomId('button_appeal_deny').setLabel('Deny').setStyle(ButtonStyle.Danger)
				...
				const responseEmbed = new EmbedBuilder()
					.setTitle('Appeal Success')
					.setDescription('Your appeal submission has been successfully sent.')
					.setColor(config.embeds.colors.success)
					.setTimestamp()
				...
				const logChannel = client.guilds.cache.get(config.guildId)?.channels.cache.get(config.logging.appeals)
				if (logChannel) {
					await logChannel.send({
						embeds: [logEmbed],
						components: [tools],
				...
				logger.debug(`Appeal submitted for user ${userId}`)
			}),
		}
		...
		if (modalHandlers[modalCustomId]) {
			try {
				...
				let appeals = getAppeals().filter(id => id !== interaction.user.id)
				saveAppeals(appeals)
