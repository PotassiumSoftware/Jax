...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('embeds')
		.setDescription('Sends either the rules, verification, or appeals embed(s).')
		.addStringOption(option =>
			option
				.setName('type')
				.setDescription('Choose the type of embed to send.')
				.setRequired(true)
				.addChoices({ name: 'Rules', value: 'rules' }, { name: 'Verification', value: 'verification' }, { name: 'Appeals', value: 'appeals' }),
		),
	...
	async execute(interaction) {
		...
		if (embedType === 'verification') {
			const finalEmbed = new EmbedBuilder()
				...
				.setDescription(
					'Welcome to our community! To gain full access, you must complete the verification process.\n\n' +
						...
						'2. Answer the required questions accurately.\n' +
						'3. A staff member will review and approve your submission.\n\n' +
			...
			const appealButton = new ButtonBuilder().setCustomId('button_appeal').setLabel('Appeal').setStyle('Primary')
